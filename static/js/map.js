// Shared warehouse network - 8 hubs spread evenly across Dehradun so the
// Dijkstra/nearest-neighbour routing engine always has multiple comparable
// alternatives to choose between. Keep in sync with WAREHOUSE_SEED_DATA in
// app.py / routes.py. Used by every map on the site so they all agree.
const DEHRADUN_WAREHOUSES = [
    { id: 1, name: 'ISBT Hub',                location: 'ISBT Dehradun',       lat: 30.3035, lng: 78.0450, products: 'Electronics, Mobile Accessories' },
    { id: 2, name: 'Clock Tower Hub',         location: 'Clock Tower',         lat: 30.3223, lng: 78.0598, products: 'Groceries, Daily Essentials' },
    { id: 3, name: 'Ballupur Hub',            location: 'Ballupur Chowk',      lat: 30.3605, lng: 78.0155, products: 'Medicines, First Aid' },
    { id: 4, name: 'Rajpur Road Hub',         location: 'Rajpur Road',         lat: 30.3350, lng: 78.1024, products: 'Books, Stationery, Gifts' },
    { id: 5, name: 'Prem Nagar Hub',          location: 'Prem Nagar',          lat: 30.3350, lng: 77.9980, products: 'Clothing, Accessories' },
    { id: 6, name: 'Clement Town Hub',        location: 'Clement Town',        lat: 30.3000, lng: 78.0044, products: 'Sports Equipment, Fitness Gear' },
    { id: 7, name: 'Sahastradhara Hub',       location: 'Sahastradhara Road',  lat: 30.3764, lng: 78.0930, products: 'Furniture, Home Decor' },
    { id: 8, name: 'Mussoorie Diversion Hub', location: 'Mussoorie Diversion', lat: 30.3891, lng: 78.0450, products: 'Pharmacy, Cosmetics, Wellness' },
];

const MAP_DEFAULT_CENTER = [30.3165, 78.0322];
...
class MapManager {
    ...
    createCustomIcons() {
        ...
        // Docked drone icon — orange
        this.droneIconDocked = L.divIcon({
    ...
    _getDroneIcon(status) {
        ...
        if (status === 'docked')      return this.droneIconDocked;
    ...
    _buildDronePopup(id, info) {
        ...
        return `
            <div class="drone-popup" style="min-width:180px;">
                ...
                ${info.warehouse_id ? '<div class="mt-1"><small class="text-muted">At warehouse</small></div>' : ''}
            </div>`;
    }
    ...
    _formatStatus(status) {
        const map = {
            ...
            delivering: 'Delivering', docked: 'Docked', low_battery: '⚠ Low Battery',
    ...
    getDroneStatusColor(status) {
        const map = {
            ...
            docked: 'warning', busy: 'warning', maintenance: 'danger'
        };
...
class DeliveryLocationMapManager extends MapManager {
    ...
    async loadWarehouses() {
        DEHRADUN_WAREHOUSES.forEach(w => this.addWarehouseMarker(w.id, w.lat, w.lng, w));
    }
}
...
class DashboardMapManager extends MapManager {
    constructor(containerId, options = {}) {
        super(containerId, { center: [30.3165, 78.0322], zoom: 12, ...options });
